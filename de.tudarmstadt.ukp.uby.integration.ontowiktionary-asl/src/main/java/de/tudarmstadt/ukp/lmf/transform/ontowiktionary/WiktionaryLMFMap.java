...
//			case UNSPECIFIED:
	 ...
	 * @param gender
	 */
	public static EGrammaticalGender mapGender(GrammaticalGender gender) {
		if (gender != null)
			switch (gender) {
				case MASCULINE:
	...
	public static WiktionaryLabelManager createLabelManager() {
		try {
			return new WiktionaryLabelManager(
					WiktionaryLMFMap.class.getClassLoader()
					.getResource(PRAGMATIC_LABELS_RESOURCE).openStream(),
					WiktionaryLMFMap.class.getClassLoader()
					.getResource(WORD_FORM_LABELS_RESOURCE).openStream());
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
	}
...
//					return ETense.future;
	...
	public static EVerbFormMood mapVerbFormMood(final IWiktionaryWordForm wordForm) {
		if (wordForm.getNonFiniteForm() == NonFiniteForm.PARTICIPLE)
			return EVerbFormMood.participle;
		if (wordForm.getNonFiniteForm() == NonFiniteForm.INFINITIVE)
			return EVerbFormMood.infinitive;
		if (wordForm.getMood() == GrammaticalMood.IMPERATIVE)
			return EVerbFormMood.imperative;
		if (wordForm.getMood() == GrammaticalMood.CONJUNCTIVE)
			return EVerbFormMood.subjunctive;
		if (wordForm.getMood() == GrammaticalMood.INDICATIVE)
			return EVerbFormMood.indicative;

		/*
		GrammaticalMood.INDICATIVE;
		GrammaticalMood.IMPERATIVE;
		GrammaticalMood.CONJUNCTIVE;
		NonFiniteForm.INFINITIVE;
		NonFiniteForm.PARTICIPLE;
		GrammaticalAspect.PERFECT;
		GrammaticalAspect.IMPERFECT;
