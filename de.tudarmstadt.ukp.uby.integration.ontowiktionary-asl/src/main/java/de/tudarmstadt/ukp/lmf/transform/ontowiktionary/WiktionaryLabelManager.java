...
public class WiktionaryLabelManager {
	...
	protected Map<String, String[]> wordFormLabels;
	...
	protected void loadLabelGroups(final InputStream inputStream) throws IOException {
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				String label = fields.remove(fields.size() - 1);
				labelGroups.put(label, fields.toArray(new String[0]));
			}
		} finally {
			reader.close();
		}
	...
	protected void loadWordForms(final InputStream inputStream) throws IOException {
			wordFormLabels = new TreeMap<String, String[]>();
			BufferedReader reader = new BufferedReader(
					new InputStreamReader(inputStream, StandardCharsets.UTF_8));
			try {
				String line;
				while ((line = reader.readLine()) != null) {
					List<String> fields = split(line);
					String label = fields.remove(0);
					wordFormLabels.put(label, fields.toArray(new String[0]));
				}
			} finally {
				reader.close();
			}
	...
	public String[] getWordFormLabel(final PragmaticLabel label) {
		if (wordFormLabels == null)
			loadWordForms();
		return wordFormLabels.get(label.getLabel());
	}
	...
	public String extractTargetWordForm(final String senseDefinition) {
		...
		while (matcher.find()) {
			...
			if (wordFormLabels.get(matches[0]) == null)
				continue;
